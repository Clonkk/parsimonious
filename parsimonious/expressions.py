...
def expression(callable, rule_name, grammar):
    ...
    if num_args == 2:
        ...
        raise RuntimeError("Custom rule functions must take either 2 or 5 arguments, not %s." % num_args)

    class AdHocExpression(Expression):
        def _uncached_match(self, text, pos, cache, error):
            result = callable(text, pos) if is_simple else callable(text, pos, cache, error, grammar)
...
class Regex(Expression):
    ...
    def _uncached_match(self, text, pos, cache, error):
        ...
        if m is not None:
            ...
            # Only store match object for small texts to avoid GC pressure.
            # Large queries (e.g. IN clauses with 1000+ values) can create
            # thousands of match objects that all reference the same large
            # input string, causing expensive GC pauses (300-400ms).
            # For texts < 2KB, the memory pressure is negligible.
            if len(text) < 2048:
                node.match = m
            return node
    ...
    def _as_rhs(self):
        return "~{!r}{}".format(self.re.pattern, self._regex_flags_from_bits(self.re.flags))
...
class Quantifier(Compound):
    ...
    def _eq_check_cycles(self, other, checked):
        return super()._eq_check_cycles(other, checked) and self.min == other.min and self.max == other.max
